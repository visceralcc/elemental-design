...
Surface is a native macOS and iOS design tool for building apps, websites, and components. It is designed for designers, developers, and people who have never used a design tool before.

**Design principle: Intent over configuration.** Surface never asks you to manually construct what it can reasonably infer. You declare what something *is* and what it *does*. Surface handles how it looks and behaves by default, and reveals controls progressively when you want to go further.

Surface is not Figma. It does not use the toolbar-and-panel metaphor. There are no tools to pick up and put down. There are no property inspectors open by default. Complexity is always one tap away — never in your face.

---

## 2. The Three Panels

Surface has three persistent panels:
...
Surface has two root object types:
...
The `>>>` indicator is Surface's core interaction pattern. It appears on every element row in the Elements panel and means: *there is more here, when you want it.*

Tapping a row expands it inline. Controls appear. Tapping again collapses it. Nothing is open by default. This is how Surface stays approachable for non-designers and deep enough for experts — the same UI, the same objects, the same tool.
...
- Not a cloud-first tool. Surface is native. Files live locally.

---

## 8. Document Structure

This file is the top of the Surface specification tree. Every major system has its own spec file. This document links to those files using the `router→` pattern.

`router→` is a navigation primitive in Surface's documentation system. When an AI agent or developer encounters `router→`, it means: *follow this link if you need depth on this topic.* Each linked file is focused, minimal, and independently readable.

All spec files live in `specs/` at the project root.

| File | Contents |
|------|----------|
| Spec_Concept_Surface.md | This file. Philosophy, model, structure. |
